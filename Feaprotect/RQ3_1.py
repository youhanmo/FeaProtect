...
class ensemble_model(tf.keras.Model):
    ...
    def call(self, v1_data, v2_data):
        ...
        if 'average' in self.ensemble:
            ...
            pred2 = []
            for _ in range(loops):
                pred1.append(tf.nn.softmax(model1_with_dropout(v1_data, training=True), axis=-1))
                pred2.append(tf.nn.softmax(model2_with_dropout(v2_data, training=True), axis=-1))
...
class fixmodel:
    def __init__(self, params) -> None:
        ...
        self.prepare_data()
        self.model2 = tf.keras.models.load_model(os.path.join('models', params.dataset.lower(), params.dataset_col, 'v2_weights.hdf5'))
    ...
    def get_ori_model_v2_acc(self, inputs, labels) -> float:
    ...
    def get_ori_model_v1_acc(self, inputs, labels) -> float:
        ori_model_v2 = tf.keras.models.load_model(
            os.path.join('models', params.dataset.lower(), params.dataset_col, 'v1_weights.hdf5'))
        ori_model_v2.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
        inputs = np.delete(inputs, self.experiment.dataset['drop_columns_ids'], axis=1)
        modelv2_acc = ori_model_v2.evaluate(inputs, labels)[1]
        return modelv2_acc
    ...
    def get_ori_acc(self) -> tuple:
        ...
        modelv2_acc = self.get_ori_model_v2_acc(v2_test_inputs.copy(), v2_test_labels.copy())
        modelv1_acc = self.get_ori_model_v1_acc(v2_test_inputs.copy(), v2_test_labels.copy())
    ...
    def ua_test(self, model, data):
        ...
        nll_criterion = tf.keras.losses.CategoricalCrossentropy()
        logits = []
        softmaxes = []
        labels = []
        for x, y in tf.data.Dataset.from_tensor_slices((inputs, targets)).batch(params.batch_size):
            y = tf.argmax(y, axis=1)
            output = model(x, training=False)
            logits.append(output)
            labels.append(y)
            if self.params.softmax == True:
                confs = tf.keras.activations.softmax(output, axis=-1)
            else:
                confs = output
            softmaxes.append(confs)
            max_values = tf.reduce_max(confs, axis=1)
            max_indices = tf.argmax(confs, axis=1)
            confs, output = max_values, max_indices
            conf += tf.reduce_sum(confs)
            indicate = (output == y)
            acc += tf.reduce_sum(tf.cast(indicate, tf.int32))
        ...
        conf = conf/total
        acc = acc/total
        softs = tf.concat(softmaxes, 0)
        targets = tf.keras.utils.to_categorical(labels, num_classes=params.num_classes)
        mse = mse_loss(softs, targets)
        ce = nll_criterion(logits, tf.cast(targets, dtype=tf.float32))

        print('Accuracy of the model: {:.4f}, Cross entropy: {:.4f}, Mean squared error: {:.4f}, '.format(acc, ce, mse))
        print('avg conf of the model: {:.4f}'.format(conf))
        return 
    ...
    def get_test_acc(self, v1_test_inputs, v2_test_inputs, test_outputs, model1, combine_model):
        ...
        old_model2_y_pred = old_model2.predict(v2_test_inputs)
        old_model_nf_index_list = []
        for idx, (v1_p, v2_p, t) in enumerate(zip(modelv1_y_pred, old_model2_y_pred, test_outputs)):
            if np.argmax(v1_p) == np.argmax(t) and np.argmax(v2_p) != np.argmax(t):
                old_model_nf_index_list.append(idx)
        
        diff_nf_index_list = list(set(new_model_nf_index_list).difference(set(old_model_nf_index_list)))
        print('diff_nf_index_list: {}'.format(diff_nf_index_list))
    ...
    def evaluate(self, combine_model) -> None:
        if self.params.rf:
            ...
            old_model2 = tf.keras.models.load_model(os.path.join('models', params.dataset.lower(), params.dataset_col, 'v2_weights.hdf5'))
            old_model2.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
            old_model2_y_pred = old_model2.predict(rf_data_input)
            confs_o = tf.reduce_max(old_model2_y_pred, axis=1).numpy()
            old_model2_y_p = tf.argmax(old_model2_y_pred, axis=1)
