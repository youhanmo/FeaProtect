...
def preprocess_data(data, sc):
    p_data = sc.transform(data)
    return p_data

def clip_input(input, experiment):
    I_new = input.copy()
    low, high, is_enum, enum_value, discrete = get_low_high(experiment.dataset['train_df'], experiment.dataset['dataset_name'])
    idx2col = {idx: str(item) for idx, item in enumerate(experiment.dataset['v2_columns'])}
    for index, value in enumerate(input):
        if is_enum[idx2col[index]]:
            I_new[index] = min(enum_value[idx2col[index]], key=lambda x: abs(x - value))
            continue
        I_new[index] = np.clip(value, low[idx2col[index]], high[idx2col[index]])

        if discrete[idx2col[index]]:
            I_new[index] = int(value)
        else:
            I_new[index] = float(value)
    return I_new