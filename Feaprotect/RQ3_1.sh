...
do
    log_name="${fix_type}_${ensemble}.log"
    log_path="results/logs/${dataset}/${dataset_col}"
    if [ ! -d "$log_path" ]; then
        mkdir -p "$log_path"
    fi
    save_path="${log_path}/${log_name}"
    nohup python -u RQ3_1.py --fix_type $fix_type --ensemble $ensemble --dataset $dataset --model $dataset --num_classes $num_classes --dataset_col $dataset_col > $save_path 2>&1 &
